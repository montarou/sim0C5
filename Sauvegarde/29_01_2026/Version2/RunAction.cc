...
void RunAction::EndOfRunAction(const G4Run*)
{
    ...
    if (isMaster) {
        ...
        // Conversion : Edep (keV) -> Dose (µGy)
        // Dose = Edep / masse
        // 1 keV = 1.602e-16 J
        // 1 Gy = 1 J/kg = 1e6 µGy
        // Donc: Dose [µGy] = Edep [keV] * 1.602e-7 / masse [g]
        constexpr G4double keV_to_uGy_per_gram = 1.602e-7;
        
        // H3: Dose totale dans l'eau (run complet)
        ...
        for (G4int i = 0; i < kNbWaterRings; i++) {
            ...
            am->FillH1(5 + i, dose_ring_run);
        }
        
        // Afficher le résumé des doses
        G4cout << "\n==================== RÉSUMÉ DOSE ====================\n";
        G4cout << "Énergie totale déposée dans l'eau : " << fTotalEdepWater << " keV\n";
        G4cout << "Dose totale dans l'eau (run)      : " << dose_total_run << " µGy\n";
        G4cout << "Dose par anneau (run) :\n";
        for (G4int i = 0; i < kNbWaterRings; i++) {
            G4double dose_ring = fTotalEdepRing[i] * keV_to_uGy_per_gram / kMassRing[i];
            G4cout << "  Anneau " << i << " (r=" << 2*i << "-" << 2*(i+1) << "mm) : "
                   << fTotalEdepRing[i] << " keV -> " << dose_ring << " µGy\n";
        }
        G4cout << "=====================================================\n";
        // ====================================================================================
...
void RunAction::AddEdepFromEvent(const G4double* edepRing, G4double edepTotal)
{
    ...
    for (G4int i = 0; i < kNbWaterRings; i++) {
        ...
        fEdepRing1000[i] += edepRing[i];
    }
    fTotalEdepWater += edepTotal;
    fEdepWater1000 += edepTotal;
}
...
{
    // Remplir les histogrammes de dose tous les 1000 événements
    if (eventID > 0 && eventID % 1000 == 0 && eventID != fLastHistoFillEvent) {
        ...
        // 1 keV = 1.602e-16 J
        // 1 Gy = 1 J/kg = 1e6 µGy
        // Donc: Dose [µGy] = Edep [keV] * 1.602e-16 / masse [kg] * 1e6
        //                  = Edep [keV] * 1.602e-10 / masse [kg]
        //                  = Edep [keV] * 1.602e-10 / (masse [g] * 1e-3)
        //                  = Edep [keV] * 1.602e-7 / masse [g]
        constexpr G4double keV_to_uGy_per_gram = 1.602e-7;
        
        // H4: Dose totale dans l'eau (par 1000 événements)
        ...
        for (G4int i = 0; i < kNbWaterRings; i++) {
            ...
            analysisManager->FillH1(10 + i, dose_ring);
        }
        
        // Réinitialiser les accumulateurs pour les prochains 1000 événements
        for (G4int i = 0; i < kNbWaterRings; i++) {
            fEdepRing1000[i] = 0.0;
        }
        fEdepWater1000 = 0.0;
        ...
        if (fRunVerbose == 1) {
            G4cout << "[DOSE] Event " << eventID << " : dose_total_1000evt = " 
                   << dose_total << " µGy" << G4endl;
        }
    }
}
