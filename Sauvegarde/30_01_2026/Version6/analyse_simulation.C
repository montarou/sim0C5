...
void analyse_simulation(const char* filename = "output.root")
{
    ...
    //==========================================================================
    // 4. HISTOGRAMMES DE DOSE PAR 1000 EVENEMENTS
    //==========================================================================
    std::cout << "\n=== Recuperation des histogrammes de dose ===" << std::endl;
    
    // Valeur max pour l'axe X en nGy (1 nGy = 0.001 muGy)
    const double doseMax_nGy = 1.0;  // 1 nGy
    const double doseMax_uGy = doseMax_nGy / 1000.0;  // = 0.001 muGy
    
    TCanvas *c9 = new TCanvas("c9", "Dose par 1000 evt", 1800, 1000);
    c9->Divide(3, 2);
    ...
    if (h_dose_total) {
        std::cout << "  Dose_total_1000evt: " << h_dose_total->GetEntries() << " entrees" 
                  << ", mean=" << h_dose_total->GetMean() 
                  << ", RMS=" << h_dose_total->GetRMS() << std::endl;
        h_dose_total->SetTitle("Dose totale par 1000 evt;Dose (nGy);Counts");
        h_dose_total->SetLineColor(kBlack);
        h_dose_total->SetLineWidth(2);
        h_dose_total->SetFillColor(kGray);
        // Convertir l'axe X de muGy en nGy (x1000)
        h_dose_total->GetXaxis()->SetLimits(h_dose_total->GetXaxis()->GetXmin()*1000, 
                                            h_dose_total->GetXaxis()->GetXmax()*1000);
        h_dose_total->GetXaxis()->SetRangeUser(0, doseMax_nGy);
        h_dose_total->Draw();
    } else {
        std::cout << "  [WARN] Dose_total_1000evt non trouve" << std::endl;
    }
    ...
    for (int i = 0; i < 5; i++) {
        ...
        if (h_dose_ring) {
            std::cout << "  Dose_ring" << i << "_1000evt: " << h_dose_ring->GetEntries() 
                      << " entrees, mean=" << h_dose_ring->GetMean() 
                      << " muGy = " << h_dose_ring->GetMean()*1000 << " nGy" << std::endl;
            h_dose_ring->SetTitle(Form("Dose anneau %d (r=%d-%d mm) par 1000 evt;Dose (nGy);Counts", 
                                       i, 2*i, 2*(i+1)));
            h_dose_ring->SetLineColor(ringColors[i]);
            h_dose_ring->SetLineWidth(2);
            h_dose_ring->SetFillColor(ringColors[i]-9);
            // Convertir l'axe X de muGy en nGy
            h_dose_ring->GetXaxis()->SetLimits(h_dose_ring->GetXaxis()->GetXmin()*1000, 
                                               h_dose_ring->GetXaxis()->GetXmax()*1000);
            h_dose_ring->GetXaxis()->SetRangeUser(0, doseMax_nGy);
            h_dose_ring->Draw();
        } else {
            std::cout << "  [WARN] Dose_ring" << i << "_1000evt non trouve" << std::endl;
        }
    }
    ...
    // Recuperer les doses depuis les histogrammes _run
    // Affichage en nGy (1 muGy = 1000 nGy)
    TH1D *h_dose_vs_ring = new TH1D("h_dose_vs_ring", 
        "Dose deposee par anneau;Numero d'anneau;Dose (nGy)", 5, -0.5, 4.5);
    
    double dose_totale_uGy = 0.0;
    double doses_uGy[5] = {0, 0, 0, 0, 0};
    double doses_nGy[5] = {0, 0, 0, 0, 0};
    
    // Recuperer la dose totale du run
    TH1D *h_total_run = (TH1D*)f->Get("Dose_total_run");
    if (h_total_run && h_total_run->GetEntries() > 0) {
        dose_totale_uGy = h_total_run->GetMean();
        std::cout << "  Dose totale: " << dose_totale_uGy << " muGy = " 
                  << dose_totale_uGy * 1000 << " nGy" << std::endl;
    }
    
    // Recuperer les doses par anneau
    for (int i = 0; i < 5; i++) {
        TH1D *h_ring = (TH1D*)f->Get(Form("Dose_ring%d_run", i));
        if (h_ring && h_ring->GetEntries() > 0) {
            doses_uGy[i] = h_ring->GetMean();
            doses_nGy[i] = doses_uGy[i] * 1000.0;  // Conversion en nGy
    ...
    pave->AddText("");
    pave->AddText(Form("#bf{D_{total} = %.4f #muGy}", dose_totale_uGy));
    pave->AddText(Form("= %.2f nGy", dose_totale_uGy * 1000));
    pave->Draw();
    ...
    pave2->AddText(Form("#bf{%.2f nGy}", dose_totale_uGy * 1000));
    pave2->Draw();
    ...
    std::cout << "  Dose totale: " << dose_totale_uGy << " muGy = " << dose_totale_uGy*1000 << " nGy" << std::endl;
    for (int i = 0; i < 5; i++) {
        std::cout << "  Anneau " << i << " (r=" << 2*i << "-" << 2*(i+1) << " mm): " 
                  << doses_uGy[i] << " muGy = " << doses_nGy[i] << " nGy" << std::endl;
    }
