...
void SetupAnalysis()
{
    ...
    // Range étendu: 0-500000 pGy (= 0-500 nGy) pour runs jusqu'à 50M événements
    analysisManager->CreateH1("Dose_total_run", 
        "Dose totale dans l'eau (run);Dose (pGy);Counts", 500, 0., 500000.);  // ID 3 (pGy)
    ...
    // Range étendu: 0-500000 pGy pour runs jusqu'à 50M événements
    analysisManager->CreateH1("Dose_ring0_run", 
        "Dose anneau 0 (r=0-2mm) run;Dose (pGy);Counts", 500, 0., 500000.);  // ID 5 (pGy)
    analysisManager->CreateH1("Dose_ring1_run", 
        "Dose anneau 1 (r=2-4mm) run;Dose (pGy);Counts", 500, 0., 500000.);  // ID 6 (pGy)
    analysisManager->CreateH1("Dose_ring2_run", 
        "Dose anneau 2 (r=4-6mm) run;Dose (pGy);Counts", 500, 0., 500000.);  // ID 7 (pGy)
    analysisManager->CreateH1("Dose_ring3_run", 
        "Dose anneau 3 (r=6-8mm) run;Dose (pGy);Counts", 500, 0., 500000.);  // ID 8 (pGy)
    analysisManager->CreateH1("Dose_ring4_run", 
        "Dose anneau 4 (r=8-10mm) run;Dose (pGy);Counts", 500, 0., 100000.);  // ID 9 (pGy)
