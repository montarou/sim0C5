...
void SetupAnalysis()
{
    ...
    //
    // Valeurs typiques pour le run complet (1M événements):
    //   - Dose totale: ~10-15 nGy = 10000-15000 pGy
    //   - Dose par anneau: ~15-20 nGy = 15000-20000 pGy
    // ============================================================================
    
    // H3: Dose totale dans tout l'eau (run complet) - en pGy
    analysisManager->CreateH1("Dose_total_run", 
        "Dose totale dans l'eau (run);Dose (pGy);Counts", 200, 0., 20000.);  // ID 3 (pGy)
    ...
    analysisManager->CreateH1("Dose_ring0_run", 
        "Dose anneau 0 (r=0-2mm) run;Dose (pGy);Counts", 200, 0., 25000.);  // ID 5 (pGy)
    analysisManager->CreateH1("Dose_ring1_run", 
        "Dose anneau 1 (r=2-4mm) run;Dose (pGy);Counts", 200, 0., 25000.);  // ID 6 (pGy)
    analysisManager->CreateH1("Dose_ring2_run", 
        "Dose anneau 2 (r=4-6mm) run;Dose (pGy);Counts", 200, 0., 25000.);  // ID 7 (pGy)
    analysisManager->CreateH1("Dose_ring3_run", 
        "Dose anneau 3 (r=6-8mm) run;Dose (pGy);Counts", 200, 0., 25000.);  // ID 8 (pGy)
    analysisManager->CreateH1("Dose_ring4_run", 
        "Dose anneau 4 (r=8-10mm) run;Dose (pGy);Counts", 200, 0., 5000.);  // ID 9 (pGy)
