...
G4bool SurfaceSpectrumSD::ProcessHits(G4Step* step, G4TouchableHistory*)
{
  ...
  // CORRECTION: L'ordre des colonnes DOIT correspondre à la définition dans
  // AnalysisManagerSetup.cc qui utilise l'ordre:
  //   0: pdg (int), 1: name (string), 2: is_secondary (int),
  //   3: x_mm (double), 4: y_mm (double), 5: z_mm (double),
  //   6: ekin_keV (double), 7: trackID (int), 8: parentID (int),
  //   9: creator_process (string)
  // ============================================================================
  if (fPassageNtupleId >= 0) {
    ...
    if (man && man->IsActive()) {
      ...
      // Déterminer si c'est une particule secondaire
      const G4int is_secondary = (parentID == 0) ? 0 : 1;

      // ============================================================================
      // [FIX] ORDRE CORRIGÉ DES COLONNES - doit correspondre à AnalysisManagerSetup.cc
      // Colonnes: pdg, name, is_secondary, x_mm, y_mm, z_mm, ekin_keV, trackID, parentID, creator_process
      // ============================================================================

      static int c=0, maxPrint=10;
      if (c < maxPrint) {
        G4cout << "[plane_passages][fill#" << (c+1) << "] id=" << fPassageNtupleId
        << " pdg="<<pdg<<" name="<<name<<" is_secondary="<<is_secondary
        << " x="<<x_mm<<" y="<<y_mm<<" z="<<z_mm
        << " E="<<E_keV<<" keV"
        << " trackID="<<trackID<<" parentID="<<parentID<<" creator="<<creator_process << G4endl;
        ++c;
      }
      
      // REMPLISSAGE DANS L'ORDRE CORRECT
      ...
      if (fRowsThisEvent <= fDbgMaxPrint || (fRowsThisEvent % 1000 == 0)) {
        auto ev = G4RunManager::GetRunManager()->GetCurrentEvent();
        G4int eid = ev ? ev->GetEventID() : -1;
        G4cout << ThreadTag() << " [SpecSD] row in event " << eid
        << " (#ev=" << fRowsThisEvent << ", tot=" << fRowsTotal << ")"
        << " E_keV=" << E_keV
        << " pos(mm)=(" << x_mm << "," << y_mm << "," << z_mm << ")"
        << G4endl;
      }

      // [ADD] Confirmation (limité)
      {
        static int dbg_write = 0;
        if (dbg_write < 20) {
          G4cout << "[SpecSD] WROTE row in plane_passages (ntupleId=" << fPassageNtupleId
          << ") pdg=" << pdg << " x=" << x_mm << " y=" << y_mm << " z=" << z_mm
          << " EkeV=" << E_keV << " name=" << name
          << G4endl;
          ++dbg_write;
        }
      }
