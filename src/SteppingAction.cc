...
void SteppingAction::UserSteppingAction(const G4Step *step)
{
    ...
    // [ADD] LV pointer trace for logicScorePlane (helps verify same LV instance)
    ...
    do {
        ...
        if ((enter || leave) && seen < maxPrint) {
            ...
            << " preLV="  << (preLV  ? preLV->GetName()  : "<none>")
            << " postLV=" << (postLV ? postLV->GetName() : "<none>")
            << G4endl;
            ++seen;
        }
    } while(0);
    ...
    do {
        ...
        if (rpre.z() <= zPlane && rpost.z() >= zPlane) {
            ...
            static int seen = 0, maxPrint = 40; // limite le spam
            if (seen < maxPrint) {
                ...
                << " preLV="  << (preLV  ? preLV->GetName()  : "<none>")
                << " postLV=" << (postLV ? postLV->GetName() : "<none>")
                << " dirZ="   << track->GetMomentumDirection().z()
                << G4endl;
                ++seen;
            }
        }
    ...
    if (analysisManager)
        analysisManager->FillH1(2, energy);  // histogramme ID 2
    }
    ...
    if (analysisManager)
        analysisManager->FillH1(3, energy);  // histogramme ID 3
    }
    ...
    if ((edep > 0.0 && edep < DBL_MAX) ||
        ...
        if (analysisManager)
            analysisManager->FillH1(4, edep);  // Histogramme ID 4 : énergie déposée
        }
    }
    }
    ...
    if (!trackInfo->HasEnteredSphere() && namePost == "logicsphereWater") {
        ...
        if (analysisManager)
            analysisManager->FillH1(0, energy);  // Histogramme 0 : entrée sphère
    ...
    if (namePre == "logicsphereWater" && namePost == "logicWaterCube") {
        if (analysisManager)
            analysisManager->FillH1(1, energy);  // Histogramme 1 : sortie sphère
    }
    ...
    if (isPrimary) {
        ...
        if (analysisManager)
        {
            ...
            analysisManager->FillH1(7, thetaDeg);
            analysisManager->FillH1(8, phiDeg);
        ...
        if (analysisManager)
        {
            if (fSteppingVerboseLevel == 1) {
                G4cout<<"\n[DEBUG SteppingAction] 🌀 Remplissage des histos →"<<G4endl;}
            analysisManager->FillH1(9, thetaDeg);
            analysisManager->FillH1(10, phiDeg);
        }
